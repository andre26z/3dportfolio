...
import { furnitureById } from "./furniture";
import type { DeckCard } from "./RoomProject";
...
// Tiny 3D turntable thumbnail of the furniture, rendered on the card face.
...
export function CardFace({ id, dim }: { id: string; dim?: boolean }) {
  ...
  const accent = KIND_COLOR[def.kind] ?? "#9a8f7a";
  return (
    <div style={{ ...face, ...(dim ? { opacity: 0.25 } : null) }}>
      ...
      <div style={{ ...preview, borderColor: `${accent}55` }}>
        <CardPreview id={id} />
      </div>
...
// The deck: one of each piece, fanned side by side. Cards are reusable —
// dragging spawns furniture without consuming the card.
...
const preview: React.CSSProperties = {
  borderRadius: 8,
  border: "1px solid",
  background: "radial-gradient(circle at 50% 40%,#fffdf8,#e4d7c1)",
  overflow: "hidden",
};
